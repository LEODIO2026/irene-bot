...
### Step 6. [실행] FVG / OB 진입 — 결정과 실행
- **진입가 (Entry)**: 15m/5m FVG의 50%(CE) 또는 OB 상단 등 정교한 타점 제시. **[중요]** 현재가에서 손익비가 안 나온다면, 반드시 OTE 구간(61.8%~78.6% 되돌림)까지 기다렸다가 진입하는 시나리오를 우선적으로 제안하세요.
- **손절가 (SL)**: **[매우 중요]** 단기 구조보다 **상위 타임프레임(4H 이상)의 주요 마디가(FVG 하단, 주요 스윙 저점 등)** 바깥에 두어 개미 털기(Stop Hunt)에 당하지 않도록 넉넉하게 설정하세요.
- **익절가 (TP)**: 최근 유동성(BSL/SSL)을 넘어 상위 구조의 고/저점 등 ICT 타깃 제시. **[필수] 최소 손익비(RR) 1:2 이상이 나오는 자리만 제안**하고, 만약 1:2가 안 나온다면 "손익비가 좋지 않으니 더 깊은 되돌림을 기다리자"고 조언하세요. 
