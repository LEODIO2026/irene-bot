...
- **진입가 (Entry)**: 15m/5m FVG의 50%(CE) 또는 OB 상단 등 정교한 타점 제시
- **손절가 (SL)**: **[매우 중요]** 단기 구조보다 **상위 타임프레임(4H 이상)의 주요 마디가(FVG 하단, 주요 스윙 저저점 등)** 바깥에 두어 개미 털기(Stop Hunt)에 당하지 않도록 넉넉하게 설정하세요.
- **익절가 (TP)**: 1차(TP1)는 최근 유동성(BSL/SSL), 최종(TP2)은 상위 구조의 고/저점 등 ICT 타깃 제시 (최소 손익비 1:1.5 이상 권장)
